  ...
  (generate
    ...
    (fn [itf seed filename]
      ...
      (let [[samples-bass-sounds (list-comp (itf.smp_add (Sample_File :name (% "sfxr-evolved-bass-%d" x) :filename (sfxr-genetics "./sfxr-bass/" (% "sfxr-evolved-bass-%d" x)))) [x (range 3)])]
            ...
            [sequences (list-comp (make-fractal-note-sequence (random.choice [16 32]) 4 :sparseness-probability (+ (* (random.random) 0.3) 0.0)) [x (range 3)])]
            ; bass variation
        ...
        (setv itf.message (+ itf.message "channels " (str (+ (max (list-comp (get g 0) [g strategy.gens])) 1)) "\n"))
