...
(import [utils [print-through get-random-sample get-random-bleep ftom mtof get-wrapped value-or-callable dir-to-samples here add-itf-message-line]])
...
(defn choose-bleep-sample [itf range]
  (if (< (random.random) 0.5)
    ; evolved type
    [(itf.smp_add (Sample_File :name (+ "sfxr-evolved-bass-" range) :filename (sfxr-genetics "./sfxr-bass/" (+ "sfxr-evolved-bass-" range)))) (get {"hi" 72 "lo" 60} range)]
    ; chip type
    [(itf.smp_add (Sample_File :name "hi-bleep" :filename (get-random-bleep "hi") :loop "sustain")) (get {"hi" 60 "lo" 48} range)]))
...
(defn main [argv]
  (generate
    argv
    (fn [itf seed filename]
      (setv itf.tempo (random.randint 175 185))
      (print itf.tempo "BPM")
      (let [[samples-drums (sum (list-comp [(itf.smp_add (Sample_File :name (% "bassdrum-evolved-%d" x) :filename (sfxr-genetics "./sfxr-drums/bassdrum" (% "bassdrum-%d" x))))
            ...
            [melody-sparseness (list-comp (+ (* (random.random) 0.8) 0.1) [x (range 3)])]
            ...
            [melody-fns-main (list-comp (make-melody-fn sample-melody sample-melody-pitch (get sequences x) (get notes-sets x) :pace 4 :volume 40) [x (range 3)])]
            [melody-fns-bass (list-comp (make-melody-fn sample-bass sample-bass-pitch (get [sequence-bass sequence-bass (get sequences 1)] x) (get notes-sets x) :pace 8 :volume 64) [x (range 3)])]
