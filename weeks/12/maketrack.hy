  ...
  (generate
    ...
    (fn [itf]
      ...
      (let [[samples-bass-sounds (list-comp (itf.smp_add (Sample_File :name (% "sfxr-evolved-bass-%d" x) :filename (sfxr-genetics "./sfxr-bass/" (% "sfxr-evolved-bass-%d" x)))) [x (range 3)])]
            ...
            [melody-sample (random.choice (+ samples-bass-sounds [sample-hi-bleep]))]
            [bass-sample (random.choice samples-bass-sounds)]

            [melody-fns-main (list-comp (make-melody-fn melody-sample 72 (get sequences x) (get notes-sets x) :pace 4 :volume 40) [x (range 3)])]
            [melody-fns-bass (list-comp (make-melody-fn bass-sample 60 (get [sequence-bass sequence-bass (get sequences 1)] x) (get notes-sets x) :pace 8 :volume 64) [x (range 3)])]
